...
class TestAsyncConnectionMethods(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_sheet(self):
        ...
        """
        drive = self._get_drive()
        _id = await drive.create_sheet(spreadsheet_id=spreadsheet_id, sheet_name='test2')
    ...
    async def test_pandas_extension(self):
        ...
        """
        await connection.setup(credentials_dir=data_dir / 'credentials.json', token_dir=data_dir / 'token.json')
        
        df = await pd.from_gsheet_async(spreadsheet_id=spreadsheet_id, sheet_name=sheet_name)
        new_column_value = str(random.random())
        df['column1'] = new_column_value
